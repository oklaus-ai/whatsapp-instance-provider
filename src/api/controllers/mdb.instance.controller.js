    ...
    try {
        if (!req.body.number) {
            ...
            if (data.phone_connected === true) {
                ...
                const number = await WhatsAppInstances[
                    req.query.key
                ].getWhatsappCode(req.body.number)
                const code = await WhatsAppInstances[
                    req.query.key
                ].instance?.sock?.requestPairingCode(number)
                return res.json({
                    error: false,
                    code: code,
                })
...
exports.qr = async (req, res) => {
    ...
    if (verifica == true) {
        ...
        if (data.phone_connected === true) {
            ...
            try {
                const qrcode = await WhatsAppInstances[req.query.key]?.instance
                    .qr
                res.render('qrcode', {
                    qrcode: qrcode,
                })
...
exports.validar = async (req, res) => {
    ...
    const existe = verifica.data.some(
        (item) => item.instance_key === req.query.key
    )
    if (existe) {
        return true
    } else {
...
exports.infoManager = async (key) => {
    try {
        ...
        return {
            error: true,
            message: 'erro ao encontrar a instância, tente ovamente',
        }
    }
}
...
exports.deleteInactives = async (req, res) => {
    ...
    for (const instance of data) {
        if (
            instance.phone_connected === undefined ||
            instance.phone_connected === false
        ) {
            const deletePromise = WhatsAppInstances[
                instance.instance_key
            ].deleteInstance(instance.instance_key)
